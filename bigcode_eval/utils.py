...
def code_generation(
    ...
    assert mode in ["python_to_c", "assertion"], "Mode must be either 'python_to_c' or 'assertion'"
...
#     system_prompt_assertion = """You are an expert code reviewer. Your task is to verify the correctness of Python code generated by another agent based on a user's prompt.

# Based on the user's prompt and the generated code, produce one or two assertions to validate that the code behaves as expected.

# **NOTE:** Your response should only contain the assertions, formatted within a markdown block.
# """
...
"""
    ...
    system_prompt_assertion = """You are an expert in Python unit testing. Your task is to generate one or two assertions to verify the correctness of the code produced by another agent, based on the user's prompt and the provided Python code.

**Instructions:**

1.  Analyze the user's prompt to understand the intended functionality of the code.
...
"""
    ...
    if mode == "python_to_c":
        conversion_prompt = [
            {"role": "system", "content": system_prompt_python_to_c},
            {"role": "user", "content": user_prompt_python_to_c.format(python_code=python_code)}
        ]

    elif mode == "assertion":
        conversion_prompt = [
            {"role": "system", "content": system_prompt_assertion},
            {"role": "user", "content": user_prompt_assertion.format(user_prompt=prompt, generated_code=python_code)}
        ]
...
def process_answer_with_conversion(
    ...
    c_code = extract_c_code_block(c_code)
    result["c_code"] = c_code

    return result

def process_assertion_with_generation(
    prompt,
    generated_code,
    model,
    tokenizer,
    ...
    """Process the generated code by creating assertions based on the user's prompt.
    Args:
        prompt: The user's original prompt.
        generated_code: The Python code generated by the model.
        model: The language model used for generating assertions.
        tokenizer: The tokenizer associated with the model.
        accelerator: The accelerator for distributed computing.
    Returns:
        A list of assertions extracted from the model's output.
    """

    assertions = code_generation(
        generated_code, 
        model, 
        tokenizer, 
        accelerator,
    ...
    if matches:
        ...
        warnings.warn("No assertion code blocks found in the generated text. Returning empty list.")
        return []